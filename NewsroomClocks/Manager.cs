...
internal class Manager : INotifyPropertyChanged
{
    ...
    private const string LaunchOnStartupInitializedKey = "LaunchOnStartupInitialized";
    ...
    private async void LoadAppSetting()
    {
        ...
        if (!localSettings.Values.ContainsKey(LaunchOnStartupInitializedKey))
        {
            // This is first run of the app. Assume that launch on startup should be set
            _isLaunchOnStartupEnabled = true;
            localSettings.Values[LaunchOnStartupInitializedKey] = true;
            UpdateStartupInSystem();
        }
        else
        {
            // This isn't first run, so the system knows if launch on startup is set or not
